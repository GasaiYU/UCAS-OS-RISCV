...
#define KERNEL_VM_START 0xffffffc000000000
...
{
    // TODO:
    return kva - KERNEL_VM_START;
}

static inline uintptr_t pa2kva(uintptr_t pa)
{
    // TODO:
    return pa + KERNEL_VM_START;
}

static inline uint64_t get_pa(PTE entry)
{
    // TODO:
    uint64_t mask = ~((~0) << 54);              //Clear the top reserved bits and set other bits 1
    return ((entry & mask) >> 10) << 12;        //Remove mode information and *4096
}
...
{
    // TODO:
    uint64_t mask = ~((~0) << 54) & ((~0) << 10);       //Clear lowest 10 bits 0
    *entry = mask & (pfn << 10);                        //Set 10-54 bits pa
}
